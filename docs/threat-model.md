...
- Before those policy checks, the state API verifies the provider's immutable demo resource facts by resource ID (price/speed/slot/tariff maths). Provider-supplied metadata cannot redefine a known plan as cheaper/faster/greener. This is integrity validation only: the API still does not search, rank, select or invoke providers.
- Mission edits, approvals and confirmation transitions use write-time compare-and-set conditions so a concurrent human edit/provider re-plan cannot race an old approval across the commitment boundary. Repeated confirmation and exact-plan approval calls are idempotent.
